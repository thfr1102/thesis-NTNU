\chapter{Method}
\label{chap:method}
This chapter will go over the methods I will be using when performing my study, and will cover how I will identify, construct and conduct my study population and sampling, the data collection, and the data analysis. Lastly I give my reasoning for why these methods are appropriate for my research questions and the procurement of desired results. For my overall method, I will take a quantitative approach. 

\section{Choice of methods}
First of all, I will conduct a qualitative literature review on the state of the art of smart homes and security awareness, along with definitions to lay the foundations for the topic.

\subsection*{What is the current security awareness level of smart home users in Norway?}
...
\subsection*{What are some of the most common pitfalls of smart home users in Norway which impose risk amplification?}
...
\subsection*{What do smart home users in Norway perceive being the highest security risks when using smart home devices?}
By identifying related work on risk perceptions, I have uncovered a sizeable amount of potential risk perceptions which I can use to verify. However, I may also need to include some qualitative questions in order to fully cover this research question. 

\section{Study population and sampling}
The data gathering methodology will include a questionnaire given to a sample of the Norwegian population. I will utilise non-probability sampling since the target group comprise people who own and/or uses smart home devices in their daily lives. Furthermore, the sample will be constructed as a convenience sample, due to the difficulty of reaching out to other sample types. I will receive assistance from my supervisors to construct the sample. When creating the sample it is important to note that the response rate is about 10-15 per cent, therefore the sample size must be ten times the expected response. 

\section{Data collection}
To collect the data for my thesis, I will be using a questionnaire as my primary quantitative research method. However, before sending out the primary questionnaire, I will be performing a pilot study. This will consist of a series of qualitative interviews with several chosen smart home users, in order to collect data for improving the main data collection. The sample from this collection will be chosen as a small convenience sample and may contain slight bias. 

In order to answer my research questions, I will create a number of hypotheses' to aid in measuring the security awareness level, as well as the most common pitfalls and highest perceived risks. Since this is a quantitative approach, I have to use close-ended questions, which means that responses are predefined. To manage this, the questions will contain a mix of nominal, ordinal, interval, and ratio measurement scales, and will be made in a way so that some questions can be combined to a security awareness score. Other question types may also be included. The distribution vector will likely be by email, and the tool for creating the questionnaire will be in-house NTNU affiliated resources, such as SelectSurvey or Nettskjema. 
...
For analysis, I will use the statistical program SPSS, and create an index with multiple indicators to evaluate the current security awareness level of the respondents. 

...
From an earlier paper \cite{rahim_et_al} we learned that few studies focuses on the security awareness of youngsters in particular. However, in an article by Gkioulos et al. \cite{Gkioulos2017} they focused specifically on youngsters, who they define as digital natives, and their security awareness level. The study utilised three distinct samples, which diverged by security competence and regional background. The results concluded that these factors significantly influenced some user behaviour, while specific areas like storage of plain-text passwords were not affected. The study focused on general usage patterns and behaviours, which may be similar to what I will utilise when assessing security awareness. 

\section{Risk perceptions}
